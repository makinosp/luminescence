...
- User Request: Build a mobile client application for the Firefly III API to manage personal finances, including transaction tracking, account management, expense categorization, and financial reporting.
- Request Type: New Project
- Scope Estimate: Multiple components within a single mobile application
- Complexity Estimate: Moderate
- Requirements Depth: Standard

## Product Goal
The product shall be a mobile client for iOS and Android that connects to an existing Firefly III server and enables end users to manage personal finance data from a mobile device.

## Primary User Scenario
1. A user installs the app on a mobile device.
2. The user enters a Firefly III server base URL and a personal access token.
3. The app validates connectivity and persists configuration locally.
4. The user accesses transaction, account, category, and reporting data through the Firefly III API.

## Functional Requirements
- FR-01: The system shall be a mobile application targeting iOS and Android devices.
- FR-02: The system shall be implemented with a TypeScript-based mobile client stack centered on React Native.
- FR-03: The system shall allow the user to configure a Firefly III server base URL.
- FR-04: The system shall allow the user to provide a Firefly III personal access token for authenticated API access.
- FR-05: The system shall use the personal access token and the configured server base URL as the only authentication artifacts handled by the mobile client.
- FR-06: The system shall store non-sensitive application settings and the configured server base URL in non-sensitive local persistence.
- FR-07: The system shall store the personal access token only in platform secure storage backed by iOS Keychain, Android Keystore, or an equivalent secure-storage abstraction.
- FR-08: The system shall require a configured server base URL and personal access token before allowing authenticated Firefly III operations.
...
- FR-13: The system shall preserve local configuration between application launches.
...
- CA-03: The project scope is limited to a mobile client and does not include backend, infrastructure, or Firefly III server changes.
...
- CA-06: No separate application database is required at this stage; local persistence is limited to client-side settings and authentication artifacts.

## Extension-Derived Requirements

### Security Baseline
- SB-01: Non-sensitive settings may use AsyncStorage or an equivalent local settings store.
- SB-02: The Firefly III personal access token shall use secure platform storage only.
...
- DD-01: Exact screen architecture and user navigation flows.
- DD-02: The precise set of reporting views and financial KPIs.
- DD-03: State management, navigation, and networking library selection.
- DD-04: The React Native secure storage library selection.
